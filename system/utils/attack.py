...
# refer to https://github.com/K3ats/MODEL.
...
def FMPA_malicious(target_model_para, nusers, w_locals, choice, p):
    decay = 1.1
    min_dis = 1000000
    target_model_para = torch.tensor(target_model_para)
    w_locals = torch.tensor(w_locals)
    ...
    while True:
        ...
        w_glob = torch.mean(w_locals_c, dim=0)  # AGR: FedAvg
        # test(w_glob)
